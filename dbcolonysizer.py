...
class DBColonySizer(object):
    ...
    def initialize (self, template_image = "./data/kernel.PNG"):
        self.kernel_file = template_image
        self.NUM_COLS = 24
        self.NUM_ROWS = 16

    def get_file_list(self,image_directory = None, template_image = "./data/kernel.PNG"):
# construct the argument parser and parse the arguments
        ...
        if check_is_image(self.kernel_file)==False:
            raise Exception("Template file "+str(self.kernel_file)+" not valid image file.")
...
#define Pandas column/index
            columns = pd.MultiIndex.from_tuples([(os.path.splitext(os.path.basename(file))[0], column_num) for column_num in xrange(self.NUM_COLS)], names=["File", 'Column'])
...
if __name__ == "__main__":
    db = DBColonySizer()
    db.initialize()
    files = db.get_file_list(image_directory = None, template_image = "./data/kernel.PNG")
