...
class DBColonySizer(object):
    ...
    def initialize(self, template_image="./example_data/kernel.PNG"):
        self.kernel_file = template_image
        self.NUM_COLS = 24
        self.NUM_ROWS = 16

    def get_file_list(self, image_directory=None,
                      template_image="./example_data/kernel.PNG"):
        ...
        ap.add_argument(
            ...
            "--template",
            required=False,
            help="Path to the example cropped plate image you want to use.",
            default=template_image)
        ap.add_argument(
            "-s",
            "--size",
            required=False,
            help="Size of your plate",
            default="384")
        ...
        self.kernel_file = str(args['template'])
        if check_is_image(self.kernel_file) == False:
            raise Exception("Template file " +
                            str(self.kernel_file) +
...
# crop an image to roughly the shape of the kernel
    ...
    def process_files(self, files):
        ...
        for file in files:
            ...
            cropped_im = self.kernel_crop(image.copy())
            colony_sizes, image_w_circles = self.get_colony_size(cropped_im)
            colony_sizes = self.normalize_edges(colony_sizes)
# define Pandas column/index
            columns = pd.MultiIndex.from_tuples([(os.path.splitext(os.path.basename(file))[
                                                0], column_num) for column_num in xrange(self.NUM_COLS)], names=["File", 'Column'])
            index = [row_num for row_num in xrange(self.NUM_ROWS)]
            if not formatted_sizes_defined:
                colony_sizes_df = pd.DataFrame(
                    data=colony_sizes, index=index, columns=columns)
                ...
                print new_column_stacked.index
                print formatted_sizes.index
...
if __name__ == "__main__":
    ...
    files = db.get_file_list(
        ...
        template_image="./data/kernel.PNG")
